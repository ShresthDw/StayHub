    ...
    return (
        <div className={`flex flex-col my-3 ${isUser ? 'items-end' : 'items-start'} max-w-full`}>
            <div className={`flex items-end gap-2 max-w-[92%] sm:max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                {/* Avatar */}
                {!isUser && (
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-teal-600 to-cyan-500 flex items-center justify-center text-white shrink-0 shadow-md shadow-teal-500/20 mb-1 ring-2 ring-white dark:ring-gray-800">
                        <BotAvatarIcon />
                    </div>
                )}
                ...
                >
                    ...
                    <div className={`text-[10px] mt-1.5 flex items-center gap-1 ${isUser ? 'text-teal-100 justify-end' : 'text-gray-400 dark:text-gray-500 justify-end'}`}>
