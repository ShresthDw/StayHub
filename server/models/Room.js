...
const getActualRoadDistance = async (userLat, userLng, roomLat, roomLng) => {
    ...
    // Geoapify routing waypoints are lon,lat (not lat,lon)
    const url =
        `https://api.geoapify.com/v1/routing` +
        `?waypoints=${userLng},${userLat}|${roomLng},${roomLat}` +
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        // Clamp: near the poles cos(lat)→0, which would make degreesLng infinite.
        // 360 is the max meaningful value (full longitude circle).
        const degreesLng = cosLat < 0.01 ? 360 : Math.min(maxDistKm / (111 * cosLat), 360);
